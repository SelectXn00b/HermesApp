...
object PatchParser {
     ...
     * Returns a Pair of (operations, error_message).
     */
    ...
    /**
     * Apply an ADD operation to the filesystem.
     */
    private fun applyAdd(op: PatchOperation, baseDir: File): Pair<Boolean, String> {
    ...
    /**
     * Apply a DELETE operation to the filesystem.
     */
    private fun applyDelete(op: PatchOperation, baseDir: File): Pair<Boolean, String> {
        val file = File(baseDir, op.filePath)
    ...
    /**
     * Apply a MOVE operation to the filesystem.
     */
    private fun applyMove(op: PatchOperation, baseDir: File): Pair<Boolean, String> {
    ...
    /**
     * Apply an UPDATE operation using simple find-and-replace on hunks.
     */
    private fun applyUpdate(op: PatchOperation, baseDir: File): Pair<Boolean, String> {
        val file = File(baseDir, op.filePath)
    ...
    /**
     * Apply all patch operations to the filesystem.
     */
        ...
        for (op in operations) {
            try {
                val (success, diff) = when (op.operation) {
                    OperationType.ADD -> applyAdd(op, baseDir)
                    OperationType.DELETE -> applyDelete(op, baseDir)
                    OperationType.MOVE -> applyMove(op, baseDir)
                    OperationType.UPDATE -> applyUpdate(op, baseDir)
                }
                if (success) {
                    when (op.operation) {
