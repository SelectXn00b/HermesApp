 ...
 *
 * Hermes adaptation: gateway server and RPC methods.
 */
...
import com.xiaomo.hermes.infra.FixedWindowRateLimiter
...
import com.xiaomo.hermes.gateway.methods.SessionMethods
import com.xiaomo.hermes.gateway.methods.ModelsMethods
import com.xiaomo.hermes.gateway.methods.ToolsMethods
import com.xiaomo.hermes.gateway.methods.SkillsMethods
import com.xiaomo.hermes.gateway.methods.ConfigMethods
import com.xiaomo.hermes.gateway.methods.TalkMethods
import com.xiaomo.hermes.gateway.methods.CronMethods
...
import com.xiaomo.hermes.hooks.loadInternalHooks
import com.xiaomo.hermes.plugins.PluginLoader
import com.xiaomo.hermes.plugins.PluginRegistry
import com.xiaomo.hermes.plugins.PluginRecordStatus
import com.xiaomo.hermes.workspace.StoragePaths
import com.xiaomo.hermes.gateway.protocol.AgentParams
import com.xiaomo.hermes.gateway.protocol.AgentWaitParams
import com.xiaomo.hermes.gateway.protocol.EventFrame
import com.xiaomo.hermes.gateway.security.TokenAuth
import com.xiaomo.hermes.gateway.websocket.GatewayWebSocketServer
        ...
        try {
            ...
            server = GatewayWebSocketServer(
                ...
                registerMethod("connect") { _ ->
                    mapOf(
                        "server" to mapOf("host" to "Hermes"),
                ...
                registerMethod("chat.send") { params ->
                    ...
                    val imageBlocks = mutableListOf<com.xiaomo.hermes.providers.llm.ImageBlock>()
                    for (att in attachments) {
                        ...
                        if (type == "image" || type == "image_url") {
                            ...
                            if (!rawBase64.isNullOrBlank()) {
                                val sanitized = com.xiaomo.hermes.media.ImageSanitizer.sanitize(
                                    base64Data = rawBase64,
                                    sourceMimeType = mimeType
                                )
                                if (sanitized != null) {
                                    imageBlocks.add(com.xiaomo.hermes.providers.llm.ImageBlock(
                    ...
                    val llmProvider = com.xiaomo.hermes.providers.UnifiedLLMProvider(context)
                    ...
                    val job = serviceScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                        ...
                        try {
                            ...
                            if (hermesEnabled) {
                                ...
                                val llmProvider = com.xiaomo.hermes.providers.UnifiedLLMProvider(context)
                                val ccAdapter = com.xiaomo.hermes.hermes.AppChatCompletionAdapter(llmProvider)
                                val tdAdapter = com.xiaomo.hermes.hermes.AppToolDispatcherAdapter(
                                    com.xiaomo.hermes.agent.tools.ToolCallDispatcher(toolRegistry, androidToolRegistry)
                                )
                                ...
                                val hermesToolSchemas = com.xiaomo.hermes.hermes.HermesBridge.convertToolDefsToHermes(allToolDefs)
                                val hermesValidNames = allToolDefs.map { it.function.name }.toSet()

                                val hermesLoop = com.xiaomo.hermes.hermes.HermesAgentLoop(
                                    server = ccAdapter,
                                ...
                                val messages = com.xiaomo.hermes.hermes.HermesBridge.convertMessagesToHermes(
                                    contextHistory
                                )
                                ...
                                text = com.xiaomo.hermes.hermes.HermesBridge.extractFinalContent(hermesResult)
                                streamJob.cancel()
                ...
                // agents.list: list available agents (Hermes only has one).
    ...
    private fun legacyContentToOpenClaw(content: Any?): List<Map<String, Any?>> {
        return when (content) {
            ...
            is List<*> -> content.mapNotNull { block ->
                when (block) {
                    is com.xiaomo.hermes.providers.ContentBlock -> when (block.type) {
