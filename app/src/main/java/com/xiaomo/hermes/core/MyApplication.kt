...
import com.xiaomo.hermes.util.ReasoningTagFilter
...
import com.xiaomo.hermes.accessibility.AccessibilityHealthMonitor
import com.xiaomo.hermes.DeviceController
import com.xiaomo.hermes.util.GlobalExceptionHandler
import com.xiaomo.hermes.workspace.StoragePaths
import com.xiaomo.hermes.util.SPHelper
import com.xiaomo.hermes.util.WakeLockManager
...
import com.xiaomo.hermes.gateway.GatewayService
import com.xiaomo.hermes.gateway.MainEntryAgentHandler
import com.xiaomo.hermes.gateway.GatewayServer
import com.xiaomo.hermes.gateway.GatewayController
import com.xiaomo.hermes.agent.session.SessionManager
import com.xiaomo.hermes.agent.skills.SkillsLoader
...
import com.xiaomo.hermes.providers.llm.toLegacyMessage
...
import com.xiaomo.hermes.core.channel.ChannelMessageProcessor
...
class MyApplication : ai.openclaw.app.NodeApp(), Application.ActivityLifecycleCallbacks {
    ...
    companion object {
        ...
        private var localGatewayChannel: com.xiaomo.hermes.gateway.LocalGatewayChannel? = null
        private var hermesGatewayChannel: com.xiaomo.hermes.hermes.bridge.HermesGatewayChannel? = null
        ...
        suspend fun startHermesGateway(context: android.content.Context) {
            val gateway = com.xiaomo.hermes.hermes.bridge.HermesChatGateway.getInstance(context)
            gateway.start()
            val runner = gateway.runner
            if (runner != null) {
                val channel = com.xiaomo.hermes.hermes.bridge.HermesGatewayChannel(context, runner)
        ...
        fun getHermesChannel(): com.xiaomo.hermes.hermes.bridge.HermesGatewayChannel? = hermesGatewayChannel
        ...
        fun handleChatBroadcast(message: String) {
            ...
            try {
                // Send local broadcast for MainActivityCompose to handle
                val intent = Intent("com.xiaomo.hermes.CHAT_MESSAGE_FROM_BROADCAST")
    ...
    override fun onCreate() {
        ...
        com.xiaomo.hermes.util.LocaleHelper.applyLanguage(this)

        MMKV.initialize(this)
        com.xiaomo.hermes.config.ProviderRegistry.init(this)
        registerActivityLifecycleCallbacks(this)

        // 初始化 ShizukuManager
        com.xiaomo.hermes.accessibility.ShizukuManager.init()
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                val termux = com.xiaomo.hermes.agent.tools.TermuxBridgeTool(applicationContext)
                if (!termux.isTermuxInstalled()) return@launch
                val status = termux.getStatus()
                if (status.ready) {
                    com.xiaomo.hermes.agent.tools.TermuxSSHPool.warmUp(applicationContext)
        ...
        com.xiaomo.hermes.ui.float.SessionFloatWindow.init(this)
    ...
    private fun initializeFileLogger() {
        try {
            com.xiaomo.hermes.logging.AppLog.init(this)
    ...
    private fun initializeCronJobs() {
        try {
            com.xiaomo.hermes.cron.CronInitializer.initialize(this)
    ...
    private fun initializeWorkspace() {
        try {
            val initializer = com.xiaomo.hermes.workspace.WorkspaceInitializer(this)
    ...
    private fun testConfigSystem() {
        try {
            ...
            // com.xiaomo.hermes.config.ConfigTestRunner.runBasicTests(this)

            // Test LegacyRepository config integration
            // com.xiaomo.hermes.config.ConfigTestRunner.testLegacyRepository(this)
    ...
    private fun startGatewayService() {
        try {
            ...
            val gwUrl = com.tencent.mmkv.MMKV.defaultMMKV()
                ?.decodeString(com.xiaomo.hermes.util.MMKVKeys.GATEWAY_URL.key, "ws://127.0.0.1:8765")
                ?: "ws://127.0.0.1:8765"
            ...
            localGatewayChannel = com.xiaomo.hermes.gateway.LocalGatewayChannel(gatewayController!!)
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                if (!feishuConfig.enabled) {
                    ...
                    Log.i(TAG, "   配置路径: ${com.xiaomo.hermes.workspace.StoragePaths.openclawConfig.absolutePath}")
                ...
                if (result?.isSuccess == true) {
                    ...
                    try {
                        ...
                        if (mainToolRegistry != null && ftr != null) {
                            ...
                            val count = com.xiaomo.hermes.agent.tools.registerFeishuTools(mainToolRegistry, ftr)
    ...
    private suspend fun processFeishuMessageWithTyping(
        ...
        try {
            ...
            if (response == "\u0000BLOCK_REPLY_ALREADY_SENT") {
                ...
                var sanitizedResponse = com.xiaomo.hermes.agent.session.HistorySanitizer
                ...
                if (event.chatType == "group") {
                    sanitizedResponse = com.xiaomo.hermes.agent.context.ContextSecurityGuard
    ...
    private suspend fun processFeishuMessage(event: com.xiaomo.feishu.FeishuEvent.Message): String {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (fc != null) {
                    try {
                        ...
                        if (feishuToolRegistry != null) {
                            ...
                            val feishuToolCount = com.xiaomo.hermes.agent.tools.registerFeishuTools(toolRegistry, feishuToolRegistry)
                ...
                val llmProvider = com.xiaomo.hermes.providers.UnifiedLLMProvider(this@MyApplication)
                ...
                val showToolCalls = feishuUiConfig?.showToolCalls ?: true
                ...
                val finalContent = com.xiaomo.hermes.util.ReplyTagFilter.strip(
    ...
    private suspend fun processWeixinMessageQueued(
        ...
        try {
            ...
            session.addMessage(com.xiaomo.hermes.providers.LegacyMessage(
            ...
            if (response.isNotBlank()) {
                ...
                if (trimmed != "NO_REPLY" && trimmed != "HEARTBEAT_OK") {
                    var sanitized = com.xiaomo.hermes.agent.session.HistorySanitizer
                        .stripControlTokensFromText(response)
                        .replace(Regex("(?:^|\\s+|\\*+)NO_REPLY\\s*$"), "")
                        .replace(Regex("(?:^|\\s+|\\*+)HEARTBEAT_OK\\s*$"), "")
                        .trim()
    ...
    private fun buildDiscordSystemPrompt(event: ChannelEvent.Message): String {
        val botName = discordChannel?.getBotUsername() ?: "Hermes Bot"
                ...
                try {
                    val adapter = object : ChannelAdapter {
                        ...
                        override fun buildSystemPrompt(): String {
                            val botName = telegramChannel?.getBotUsername() ?: "Hermes Bot"
            ...
            val job = GlobalScope.launch(Dispatchers.IO) {
                try {
                    ...
                    val adapter = object : ChannelAdapter {
                        ...
                        override fun buildSystemPrompt(): String {
                            val botName = slackChannel?.getBotUsername() ?: "Hermes Bot"
...
现在，请处理用户的消息。
     ...
     */
    private fun cleanupToolMessages(messages: List<com.xiaomo.hermes.providers.LegacyMessage>): List<com.xiaomo.hermes.providers.LegacyMessage> {
