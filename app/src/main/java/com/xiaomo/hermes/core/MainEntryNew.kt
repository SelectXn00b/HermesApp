...
import com.xiaomo.hermes.ext.mmkv
import com.xiaomo.hermes.ext.simpleSafeLaunch
import com.xiaomo.hermes.providers.llm.toNewMessage
import com.xiaomo.hermes.providers.llm.toLegacyMessage
import com.xiaomo.hermes.accessibility.service.AccessibilityBinderService
import com.xiaomo.hermes.util.LayoutExceptionLogger
import com.xiaomo.hermes.util.MMKVKeys
import com.xiaomo.hermes.util.WakeLockManager
import com.xiaomo.hermes.util.ReasoningTagFilter
import com.xiaomo.hermes.ui.float.SessionFloatWindow
import com.xiaomo.hermes.BuildConfig
...
object MainEntryNew {
    ...
    const val ACTION_AGENT_PROGRESS = "com.xiaomo.hermes.ACTION_AGENT_PROGRESS"
    ...
    private var subagentSpawner: com.xiaomo.hermes.agent.subagent.SubagentSpawner? = null
    ...
    fun initialize(app: Application) {
        ...
        try {
            ...
            val llmProvider = com.xiaomo.hermes.providers.UnifiedLLMProvider(application)
            ...
            val memoryManager = com.xiaomo.hermes.agent.memory.MemoryManager(
            ...
            val workspaceDir = com.xiaomo.hermes.workspace.StoragePaths.workspace.also {
                it.mkdirs()
            ...
            val subagentsConfig = config.agents?.defaults?.subagents
                ?: com.xiaomo.hermes.config.SubagentsConfig()
            if (subagentsConfig.enabled) {
                val registryStore = com.xiaomo.hermes.agent.subagent.SubagentRegistryStore()
                val registry = com.xiaomo.hermes.agent.subagent.SubagentRegistry(registryStore)
                registry.restoreFromDisk()
                val spawner = com.xiaomo.hermes.agent.subagent.SubagentSpawner(
                    registry = registry,
                    configLoader = configLoader,
                    llmProvider = llmProvider,
                    toolRegistry = toolRegistry,
                ...
                val subagentTools = com.xiaomo.hermes.agent.subagent.SubagentSpawner.buildSubagentTools(
                    spawner = spawner,
    ...
    fun runWithSession(
        ...
        val allMessages = if (dmHistoryLimit != null && dmHistoryLimit > 0) {
            ...
            com.xiaomo.hermes.agent.session.HistorySanitizer
        ...
        val job = scope.simpleSafeLaunch(
            {
                ...
                com.xiaomo.hermes.gateway.GatewayServer.broadcastChatMessage(
                ...
                val cleanFinalContent = com.xiaomo.hermes.util.ReplyTagFilter.strip(
                ...
                if (cleanFinalContent.isNotEmpty()) {
                    ...
                    com.xiaomo.hermes.ui.float.SessionFloatWindow.updateLatestMessage(cleanFinalContent)
            ...
            { exception ->
                ...
                val errorMessage = buildString {
                    ...
                    // 如果是 LLM 异常，添加更详细的信息
                    if (exception is com.xiaomo.hermes.providers.LLMException) {
                        append("**类型**: API 调用失败\n")
