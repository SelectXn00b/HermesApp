...
import com.xiaomo.hermes.ui.activity.*
import com.xiaomo.hermes.ui.activity.LegalActivity
import com.xiaomo.hermes.ui.float.SessionFloatWindow
import ai.openclaw.app.avatar.FloatingAvatarService
import com.xiaomo.hermes.updater.AppUpdater
...
fun HermesSettingsTab() {
    ...
    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            ...
            gatewayRunning = com.xiaomo.hermes.core.MyApplication.isGatewayRunning()
    ...
    Column(
        ...
        StatusCard(
            ...
            onClick = {
                context.startActivity(Intent().apply {
                    setClassName(context, "com.xiaomo.hermes.ui.activity.ChannelListActivity")
                })
            },
        ...
        // MCP Server
        val mcpRunning = remember { mutableStateOf(com.xiaomo.hermes.mcp.ObserverMcpServer.isRunning()) }
        StatusCard(
            ...
            rows = listOf(
                ...
                StatusRow(stringResource(R.string.connect_port_label), "${com.xiaomo.hermes.mcp.ObserverMcpServer.DEFAULT_PORT}"),
            ),
            onClick = {
                context.startActivity(Intent(context, com.xiaomo.hermes.ui.activity.McpConfigActivity::class.java))
            },
        ...
        StatusCard(
            ...
            onClick = {
                try {
                    ...
                    context.startActivity(Intent(context, com.xiaomo.hermes.ui.activity.PermissionsActivity::class.java))
                }
            },
...
private fun injectRiveDisplayConfig() {
    try {
        val config = com.xiaomo.hermes.config.ConfigLoader.getInstance().loadOpenClawConfig().rive
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(vertical = 4.dp)) {
            ...
            AboutRow(stringResource(R.string.settings_github), stringResource(R.string.settings_github_desc), onClick = {
                try {
                    context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(
                        "https://github.com/SelectXn00b/Hermes"
                    )))
                } catch (_: Exception) {
                    android.widget.Toast.makeText(context, context.getString(R.string.settings_cannot_open_link), android.widget.Toast.LENGTH_SHORT).show()
                }
