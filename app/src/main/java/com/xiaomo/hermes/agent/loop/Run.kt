...
import com.xiaomo.hermes.agent.subagent.SubagentPromptBuilder
import com.xiaomo.hermes.infra.BackoffPolicy
import com.xiaomo.hermes.infra.computeBackoff
import com.xiaomo.hermes.infra.sleepWithAbort
import com.xiaomo.hermes.infra.GlobalEventBus
import com.xiaomo.hermes.routing.parseAgentSessionKey
 ...
 *
 * Hermes adaptation: iterative agent loop, tool calling, progress updates.
...
import com.xiaomo.hermes.agent.context.ContextErrors
import com.xiaomo.hermes.agent.context.ContextManager
import com.xiaomo.hermes.agent.context.ContextRecoveryResult
import com.xiaomo.hermes.agent.context.ContextWindowGuard
import com.xiaomo.hermes.agent.context.ToolResultContextGuard
...
import com.xiaomo.hermes.agent.tools.ToolCallDispatcher
import com.xiaomo.hermes.agent.tools.ToolRegistry
import com.xiaomo.hermes.providers.ChunkType
import com.xiaomo.hermes.providers.LLMResponse
import com.xiaomo.hermes.providers.LLMToolCall
import com.xiaomo.hermes.providers.LLMUsage
import com.xiaomo.hermes.providers.StreamChunk
...
import com.xiaomo.hermes.providers.llm.systemMessage
import com.xiaomo.hermes.providers.llm.userMessage
import com.xiaomo.hermes.providers.llm.assistantMessage
import com.xiaomo.hermes.providers.llm.toolMessage
import com.xiaomo.hermes.util.LayoutExceptionLogger
...
class AgentLoop(
    ...
    @Volatile private var _allToolDefinitionsCache: List<com.xiaomo.hermes.providers.ToolDefinition>? = null
    private val allToolDefinitions: List<com.xiaomo.hermes.providers.ToolDefinition>
        get() {
    ...
    private suspend fun runInternal(
        ...
        val imageRefs = com.xiaomo.hermes.agent.tools.ImageLoader.detectImageReferences(userMessage, workspaceDir)
        if (imageRefs.isNotEmpty()) {
            writeLog("🖼️ Detected ${imageRefs.size} image reference(s) in user message")
            val loadedImages = imageRefs.mapNotNull { ref ->
                com.xiaomo.hermes.agent.tools.ImageLoader.loadImageFromPath(ref)
            }
        ...
        while (!shouldStop) {
            ...
            try {
                ...
                val maxTurns = com.xiaomo.hermes.agent.session.HistoryTurnLimiter
                ...
                try {
                    ...
                    if (timeoutCompactionAttempts < MAX_TIMEOUT_COMPACTION_ATTEMPTS &&
                        ...
                        val hookCtx = com.xiaomo.hermes.agent.hook.HookContext(
                            sessionKey = null,
                            agentId = null,
                            provider = "",
                            model = modelRef ?: ""
                ...
                if (response.toolCalls != null && response.toolCalls.isNotEmpty()) {
                    ...
                    messages.add(
                        assistantMessage(
                            ...
                            toolCalls = response.toolCalls.map {
                                com.xiaomo.hermes.providers.llm.ToolCall(
                                    id = it.id,
                                    name = it.name,
                    ...
                    for (toolCall in response.toolCalls) {
                        ...
                        val toolHookCtx = com.xiaomo.hermes.agent.hook.HookContext(
                            sessionKey = null,
                            agentId = null,
                            provider = "",
                            model = modelRef ?: ""
                ...
                if (isContextOverflow && contextManager != null) {
                    ...
                    finalContent = buildString {
                        if (isBilling) {
                            ...
                            when (e) {
                                is com.xiaomo.hermes.providers.LLMException -> {
        ...
        if (effectiveFinalContent.isNotEmpty()) {
            messages.add(com.xiaomo.hermes.providers.llm.Message(
                role = "assistant",
    ...
    private fun sanitizeToolCalls(toolCalls: List<LLMToolCall>): List<LLMToolCall> {
        ...
        return com.xiaomo.hermes.agent.loop.sanitizeToolCalls(toolCalls, allowedToolNames, ::writeLog)
    }
