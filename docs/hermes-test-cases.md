...
CORE 域的两条顶层约束（R-CORE-001 1:1 对齐 / R-CORE-002 冲突以 Hermes 为准）是**元需求**，不走 JUnit 单测——由 CLAUDE.md §2 的四件套对齐脚本守护。此处登记四个脚本作为 CORE 的验收手段。
...
| TC-CORE-002-a | R-CORE-002 | `python3.11 scripts/check_reverse.py --hermes $H --android $A` | 多余类 0（方法/常量自 2026-04-26 起放宽，见 CLAUDE.md §2） | alignment | `scripts/hermes-align/scripts/check_reverse.py` ✅ |
...
| TC-MCP-010-b | R-MCP-002 | 方法签名与 `mcp_serve.py` 1:1 | 由 verify_align / check_reverse 守卫 | alignment | §2 四件套 ✅ |
